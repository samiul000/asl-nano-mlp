/*
 * Sign Language Recognition on Arduino Nano
 * 
 * This firmware receives 42-dimensional feature vectors over serial,
 * runs a small MLP model to classify the sign, and displays the result
...
#define START_CHAR '<'
#define END_CHAR   '>'
#define RESPOND              // uncomment to disable response packets

// ---- inference buffers (total ~200 B) ----
int16_t  input[L0_IN];       // 84 B — features from serial
int16_t  hidden1[L0_OUT];    // 64 B — ReLU layer 0
int16_t  hidden2[L1_OUT];    // 32 B — ReLU layer 1
float    output[L2_OUT];     // 20 B — logits
...
// ---- forward pass (layer-streaming, int32 accumulate) ----
void mlp_infer() {
    unsigned long t0 = micros();

    // layer 0: input(42 int16) × W0(42×32 int16) → hidden1(32 int16)
    ...
    // layer 1: hidden1(32 int16) × W1(32×16 int16) → hidden2(16 int16)
    ...
    // layer 2: hidden2(16 int16) × W2(16×5 int16) → output(5 float)
...
// ---- softmax approximation (max-subtract, float exp) ----
// for 5 classes, just use soft-float exp — negligible cost
float expf_approx(float x) {
    // standard minimax for small range; fallback to compiler soft-float
    return exp(x);
}
...
void loop() {
    ...
    if (pktReady) {
        ...
        if (parse_packet()) {
            ...
            frameCount++;
#ifdef RESPOND
            // send response: {sign,infer_us,confidence,fps,free_ram}
            Serial.print('{');
